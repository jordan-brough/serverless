  ...
  compileFunction(functionName) {
    return BbPromise.try(() => {
      ...
      const ReservedConcurrency =
        toNum(functionObject.reservedConcurrency) ||
        toNum(this.serverless.service.provider.reservedConcurrency) ||
        null;
      ...
      functionResource.Properties.ReservedConcurrency = ReservedConcurrency;
      ...
      this.serverless.service.functions[functionName].reservedConcurrency = ReservedConcurrency;
...
function toNum(val) {
  return (val || val === 0) ? Number(val) : val
}
